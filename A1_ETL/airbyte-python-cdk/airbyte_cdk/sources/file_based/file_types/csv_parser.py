...
class _CsvReader:
    ...
    def _get_headers(self, fp: IOBase, config_format: CsvFormat, dialect_name: str) -> List[str]:
        ...
        headers, _ = self._read_and_validate_headers(fp, config_format, dialect_name)
        return headers

    def _read_and_validate_headers(
        self, fp: IOBase, config_format: CsvFormat, dialect_name: str
    ) -> Tuple[List[str], List[str]]:
        """Reads headers from file, validates, strips trailing empties, and resets fp to 0.

        Returns (clean_headers, raw_headers). For user-provided or autogenerated headers,
        raw_headers == clean_headers (no trailing empties to strip).
        """
        if isinstance(config_format.header_definition, CsvHeaderUserProvided):
            fp.seek(0)
            columns = config_format.header_definition.column_names
    ...
    def _validate_trailing_headers(self, raw_headers: List[str]) -> List[str]:
        """Validates header names and strips trailing empty headers."""
        trailing_empty_header_count = self._get_trailing_empty_header_count(raw_headers)
        headers = (
            raw_headers[: len(raw_headers) - trailing_empty_header_count]
            if trailing_empty_header_count
            else raw_headers
        )

        if trailing_empty_header_count == len(raw_headers):
            raise AirbyteTracedException(
                message="CSV header row contains only empty column name(s).",
                internal_message=f"Found {trailing_empty_header_count} empty/whitespace-only column name(s) in header: {raw_headers}",
                failure_type=FailureType.config_error,
            )

        interior_empty_count = sum(1 for h in headers if self._is_empty_csv_value(h))
        if interior_empty_count:
            raise AirbyteTracedException(
                message="CSV header row contains interior empty column name(s).",
                internal_message=f"Found {interior_empty_count} empty/whitespace-only column name(s) before trailing blank columns in header: {raw_headers}",
                failure_type=FailureType.config_error,
            )

        return headers

    def _strip_trailing_empty_header_values(
        self,
        values: List[str],
        headers: List[str],
        ...
        strip_start = len(headers)
        strip_end = strip_start + trailing_empty_header_count
        trailing_values = values[strip_start:strip_end]
        non_empty_trailing_values = [
            value for value in trailing_values if not self._is_empty_csv_value(value)
        ]
        if non_empty_trailing_values:
            raise AirbyteTracedException(
                message="CSV data row contains non-empty value(s) under trailing empty header column(s).",
                internal_message=f"Found {len(non_empty_trailing_values)} non-empty value(s) in stripped trailing column(s) on line {lineno} of file {filename}.",
                failure_type=FailureType.config_error,
            )

        return values[:strip_start] + values[strip_end:]

    @staticmethod
    def _row_to_dict(headers: List[str], values: List[str]) -> Dict[Any, Any]:
        row: Dict[Any, Any] = dict(zip(headers, values))
    ...
    @staticmethod
    def _get_trailing_empty_header_count(headers: List[str]) -> int:
        trailing_empty_header_count = 0
        for header in reversed(headers):
            if not _CsvReader._is_empty_csv_value(header):
                break
            trailing_empty_header_count += 1
        return trailing_empty_header_count

    @staticmethod
    def _is_empty_csv_value(value: Optional[str]) -> bool:
