#!/usr/bin/env bash
# One-shot start: Dify + adapter (background)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
bash "$ROOT/scripts/mac/start-dify.sh"
...
sleep 2
bash "$ROOT/scripts/mac/healthcheck.sh" || true
echo "Stop: bash scripts/mac/stop-all.sh"
