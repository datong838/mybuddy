...
class WebAppSiteResponse(ResponseModel):
    ...
    def from_app_site(
        ...
        *,
        ...
        custom_config = None
        # Always expose remove_webapp_brand so OSS delivery can hide POWERED BY without enterprise billing.
        # Logo replacement URL still gated by can_replace_logo.
        remove_webapp_brand = bool(tenant.custom_config_dict.get("remove_webapp_brand", False))
        replace_webapp_logo = None
        if can_replace_logo and tenant.custom_config_dict.get("replace_webapp_logo"):
            replace_webapp_logo = f"{dify_config.FILES_URL}/files/workspaces/{tenant.id}/webapp-logo"
        if remove_webapp_brand or can_replace_logo:
