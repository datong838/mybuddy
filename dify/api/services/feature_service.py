...
class FeatureService:
    ...
    def get_system_features(cls, is_authenticated: bool = False) -> SystemFeatureModel:
        ...
        # AOS delivery white-label: hide Help(?) and expose brand title without enterprise license.
        # Set AOS_WHITE_LABEL=true in dify/docker/.env (see docs/palantier/10_v01/10g-*.md).
        aos_white_label = os.getenv("AOS_WHITE_LABEL", "").strip().lower() in ("1", "true", "yes", "on")
        if aos_white_label:
            system_features.branding.enabled = True
            system_features.branding.application_title = cls._aos_brand_title()
    ...
    @classmethod
    def _aos_brand_title(cls) -> str:
        """Product title for white-label; tolerate broken Windows .env encodings."""
        # 交付默认中文名写在源码里（UTF-8），避免 Windows Docker 读 .env 中文乱码。
        default = "知识顾问"
        allowed_cjk = {"知识顾问", "知识引擎"}
        raw = os.getenv("AOS_BRAND_TITLE", "").strip()
        if not raw:
            return default
        # ASCII 品牌码（如 MyBuddy）可走环境变量
        if raw.isascii() and raw.lower() not in {"?", "??", "???"}:
            return raw
        if raw in allowed_cjk:
            return raw
        # 其余含 CJK 的多半是 .env 编码损坏，忽略并回落默认
        return default
