...
const useKnowledgeDatasetSelection = ({
  ...
  useEffect(() => {
    void (async () => {
      ...
      // Prune ghost IDs: API only returns datasets that still exist; persist that list
      // so deleted KB UUIDs cannot survive save/publish while UI shows a clean list.
      ...
      if (datasetIds.length > 0) {
        ...
        resolvedDatasets = dataSetsWithDetail
        setSelectedDatasets(dataSetsWithDetail)
      }

      const nextInputs = produce(currentInputs, (draft) => {
        draft.dataset_ids =
          datasetIds.length > 0 ? resolvedDatasets.map((dataset) => dataset.id) : datasetIds
      })
